...
function addVirus(maxCellRadius) {
  ...
  var mass = util.randomInRange(config.virus.defaultMass.from, config.virus.defaultMass.to),
...
function movePlayer(player) {
  ...
  var viewCenter = new V(),
      bounds = { minX: config.gameWidth, minY: config.gameHeight, maxX: 0, maxY: 0 };
  ...
  var curve = twoRatesCurve(1.4, Math.max(config.gameWidth, config.gameHeight), 1.1),
...
function explodeCell(currentPlayer, cell, virus) {
  function explode(cell) {
      ...
      var partsMass = config.defaultPlayerMass * 2,
...
function doSplitCell(currentPlayer?:any, cell?:any, alternateSpeed?:any, slowDownStep?:any) {
...
function treatCollisions(currentPlayer) {
  ...
  for (var idx = 0; idx < massFood.length; idx += 1) {
      var mass = massFood[idx],
          ...
          massFoodNeighborhoodCircle = new C(mass, util.massToRadius(config.virus.splitMass)),
