...
console.log(s);
var tree = QuadTree(0, 0, config.gameWidth, config.gameHeight, { maxchildren: 10 });
var users = [];
var massFood = [];
var foodArray = [];
var mainTree = QuadTree(0, 0, config.gameWidth, config.gameHeight, { maxchildren: 10 });
var virusArray = [];
var sockets = {};
var leaderboard = [];
...
function addFood(toAdd) {
    ...
    while (toAdd--) {
        var position = util.uniformPosition(foodArray, radius, config.foodPlacementUniformityLevel);
        if (!position) {
...
function addVirus(maxCellRadius) {
    function anyoneNotAccepted(cell) {
        var cellCircle = new C(cell, cell.radius), collision = new SAT.Response();
    ...
    var mass = util.randomInRange(config.virus.defaultMass.from, config.virus.defaultMass.to), radius = util.massToRadius(mass);
    var keepAwayPoints = Array.prototype.concat.apply(virusArray, users.map(function (u) { return u.cells; })), attempts = 10, position, virusCircle, largestMinDistance = 0, bestPosition;
    do {
        position = util.uniformPosition(keepAwayPoints, radius, config.virusPlacementUniformityLevel);
...
function movePlayer(player) {
    if ((player === null || player === void 0 ? void 0 : player.type) === "spectate") {
        return;
    }
    for (var i = 0; i < player.cells.length; i++) {
        ...
        if (cell.speed > 6.25 && (cell.moveY || cell.moveX) && cell.slowDownStep) {
            ...
            var fenceDist = 50 + cell.radius, dist = Math.sqrt(target.y * target.y + target.x * target.x);
            if (dist < fenceDist) {
        ...
        if (cell.moveX || cell.moveY) {
            ...
            for (var j = 0; j < player.cells.length; j++) {
                if (j != i && player.cells[i] !== undefined) {
                    ...
                    if (distance < radiusTotal) {
                        ...
                        else if (player.lastSplit > new Date().getTime() - 1000 * config.mergeTimer) {
            ...
            if (cell.pushAwayFactor < 1.0) {
                cell.pushAwayFactor += config.pushAwayFactorStep;
            }
    ...
    var viewCenter = new V(), bounds = { minX: config.gameWidth, minY: config.gameHeight, maxX: 0, maxY: 0 };
    ...
    var curve = twoRatesCurve(1.4, Math.max(config.gameWidth, config.gameHeight), 1.1), spanX = curve(bounds.maxX - bounds.minX), spanY = curve(bounds.maxY - bounds.minY), aspectRatio = player.screenWidth / player.screenHeight;
    player.viewZoom = Math.max(config.minViewZoom, spanX, spanX / aspectRatio, spanY, spanY * aspectRatio);
}
function moveMass(mass) {
    ...
    var borderCalc = mass.radius + 5, bounceLimits = {
        xMin: borderCalc,
        yMin: borderCalc,
        xMax: config.gameWidth - borderCalc,
        yMax: config.gameHeight - borderCalc
    };
...
function computeMaxCellsRadius() {
    ...
    function maxCellRadius(player) {
        ...
        player.cells.map(radius).reduce(max, util.massToRadius(config.defaultPlayerMass));
    }
    return users.map(maxCellRadius).reduce(max, util.massToRadius(config.defaultPlayerMass));
}
function balanceMass() {
    var keepOutRadius = computeMaxCellsRadius();
    var allocatedTotalMass = foodArray.length * config.foodMass +
        users
    ...
    var missingFoodSlots = config.maxFood - foodArray.length;
    var availableFoodSlots = Math.floor((config.gameMass - allocatedTotalMass) / config.foodMass);
    ...
    while (0 < foodToAdd--) {
        addFood(1);
        availableFoodSlots -= config.foodMass;
        if (virusArray.length / config.maxVirus < foodArray.length / config.maxFood &&
            virusArray.length < config.maxVirus && availableFoodSlots >= config.virus.defaultMass.to) {
    ...
    var virusToAdd = config.maxVirus - virusArray.length;
    while (0 < virusToAdd-- && availableFoodSlots >= config.virus.defaultMass.to) {
        availableFoodSlots -= addVirus(keepOutRadius);
    }
}
function explodeCell(currentPlayer, cell, virus) {
    function explode(cell) {
        if (cell.mass < config.defaultPlayerMass * 4) {
            return; // can't make two parts of twice the default mass: abort
        }
        var remainingCellsCount = config.limitSplit - currentPlayer.cells.length;
        if (remainingCellsCount <= 0) {
            return; // Already divided into too manyparts: just eat the virus
        }
        var partsMass = config.defaultPlayerMass * 2, partsCount = Math.min(Math.max(0, Math.floor(cell.mass / partsMass) - 2), remainingCellsCount), splitAfterExplode = 0;
        ...
        if ((cell.mass - partsMass * partsCount) > 6 * partsMass) {
            partsMass = config.defaultPlayerMass * 3;
        ...
        }, playerAngle = Math.atan2(playerMove.y, playerMove.x), partAngle;
        var idx;
        for (idx = 0; idx < partsCount; idx++) {
            ...
            currentPlayer.cells.push({
                ...
                slowDownStep: config.explodeSlowdownStep,
                pushAwayFactor: 0.0
            });
        }
    ...
    if (currentPlayer.cells.length >= config.limitSplit) {
        return; // Already divided into too manyparts: just eat the virus
    }
    if (currentPlayer.massTotal < config.defaultPlayerMass * 4) {
        return; // can't make two parts of twice the default mass: abort
    }
    explode(cell);
...
function doSplitCell(currentPlayer, cell, alternateSpeed, slowDownStep) {
    ...
    var slowDown = slowDownStep ? slowDownStep : config.splitSlowdownStep;
    if (cell.mass < config.defaultPlayerMass * 4) {
        return;
    }
...
io.on('connection', function (socket) {
    ...
    socket.on('gotit', function (player) {
        ...
        if (util.findIndex(users, player.id) > -1) {
            ...
            if (type === 'player') {
                ...
                player.viewZoom = Math.max(config.gameWidth, config.gameHeight);
            }
            player.hue = Math.round(Math.random() * 360);
            currentPlayer = player;
            currentPlayer.lastHeartbeat = new Date().getTime();
            users.push(currentPlayer);
            io.emit('playerJoin', { name: currentPlayer === null || currentPlayer === void 0 ? void 0 : currentPlayer.name });
    ...
    socket.on('var', function (data) {
        if (config.debugLevel <= 0) {
            console.log("[WARN] player [" + currentPlayer.name + "] is trying to set variable," +
                "but debug mode is disabled on this server, with debugLevel=[" + config.debugLevel + "].");
            return;
        }
        if (!currentPlayer.admin) {
        ...
        if (data.length > 1) {
            ...
            if (varName === "mass") {
                var addedMass = varValue - currentPlayer.massTotal, firstCell = currentPlayer.cells[0];
            ...
            if (typeof varValue === "number") {
                var fraction = String(varValue - (~~varValue)), fractionDigits = fraction.length - fraction.indexOf(".") - 1;
                if (fractionDigits > 4) {
    ...
    socket.on('1', function () {
        ...
        for (var i = 0; i < currentPlayer.cells.length; i++) {
            ...
            if ((config.fireFood > 0 && (currentCell.mass >= config.defaultPlayerMass * 4)) || (config.fireFood === 0 && currentCell.mass >= config.defaultPlayerMass * 2)) {
                var masa = 1;
...
function treatCollisions(currentPlayer) {
    function eatFoodOrVirus(thing) {
        ...
        var type = thing.id.charAt(0), idx;
        switch (type) {
            case "F":
                var food = thing;
    ...
    function collectEatenOtherCells(otherCell) {
        ...
        var collision = new SAT.Response(), collided = SAT.testCircleCircle(cellCircle, new C(otherCell, otherCell.radius), collision);
        if (!collided) {
            return true; // continue iterating in the quadtree
        }
    ...
    for (var z = 0; z < currentPlayer.cells.length; z++) {
        var currentCell = currentPlayer.cells[z], cellCenter = new V().copy(currentCell), cellCircle = new C(cellCenter, currentCell.radius);
        var masaGanada = 0, cellBoundingBoxSearchArea = cellCircle.boundingBoxAsSearchArea();
    ...
    function feedVirus(virus) {
        var collision = new SAT.Response(), collided = SAT.testCircleCircle(new C(virus, virus.radius), massFoodCircle, collision);
        ...
        if (virus.mass > config.virus.splitMass) {
    ...
    for (var idx = 0; idx < massFood.length; idx += 1) {
        var mass = massFood[idx], massFoodCircle = new C(mass, mass.radius), massFoodNeighborhoodCircle = new C(mass, util.massToRadius(config.virus.splitMass)), searchArea = massFoodNeighborhoodCircle.boundingBoxAsSearchArea();
...
function moveloop() {
    ...
    for (var idx = 0; idx < users.length; idx += 1) {
        if (users[idx].lastHeartbeat < new Date().getTime() - config.maxHeartbeatInterval) {
            sockets[users[idx].id].emit('kick', 'Last heartbeat received over ' + config.maxHeartbeatInterval + 'ms ago.');
...
function rotateProfiling(name) {
    ...
    if (!config.profilingSeries || !config.profiling) {
        return;
    }
    var serie = profiling[name].serie, median = Math.round((serie.reduce(sum, 0) / serie.length) * 10) / 10, stdDev = Math.round(Math.sqrt(serie.reduce(sumDistanceToMedianSq, 0) / serie.length) * 10) / 10;
    logDebug(1, "[PROFILING] " + padded(11, name) + ": " +
        "med:" + padded(5, median) + ", stdDev:" + padded(5, stdDev) + ", " +
        "[ " + serie.map(printFigure).join(",") + " ]");
    var distrib = profiling[name].distrib, roundedVal = Math.round(serie[0]);
    ...
    serie.unshift(0);
    if (serie.length > config.profilingSerieMaxLength) {
        serie.pop();
    }
}
function printDistribution(name) {
    if (!config.profilingDistribs || !config.profiling) {
        return;
    }
    ...
    if (users.length <= 0 || distrib.count < config.profilingDistribCollectSec) {
        return;
    }
    var idx;
    ...
    var ninethDecile = distrib.length - 1, accu = 0;
...
function sendUpdates() {
    ...
    function sendUpdatesForUser(u) {
        ...
        var largestDimension = Math.max(u.screenWidth, u.screenHeight), viewWidth = Math.round(u.screenWidth * u.viewZoom / largestDimension), viewHeight = Math.round(u.screenHeight * u.viewZoom / largestDimension), viewArea = {
            x: u.x - viewWidth / 2,
            y: u.y - viewHeight / 2,
            w: viewWidth,
            h: viewHeight
        ...
        var visibleFood = [];
        mainTree.get(viewArea, util.massToRadius(config.foodMass), collectVisibleFood);
        var visibleViruses = [];
        mainTree.get(viewArea, util.massToRadius(config.virus.splitMass), collectVisibleVirus);
...
// Rutas para usuarios
// app.get("/listado", UsuariosListController);
// app.post("/guardar", UsuariosGuardarController);
